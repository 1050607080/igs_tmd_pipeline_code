...
def encode_Lua( app_android_root  ):
    tmpFolder = os.path.join( app_android_root, folder + "tmp/" )
    # encode .lua to .luac
    cmdStr = "/Library/Frameworks/Python.framework/Versions/2.7/bin/python " + os.path.normpath( os.path.join( app_android_root, "../../../Cocos2d-x/tools/cocos2d-console/bin/cocos.py " ))
    luacompileStr = "luacompile -s " + tmpFolder + " -d " + tmpFolder + " -e -k " + ENCRYPT_KEY + " -b " + SIGNATURE

    if args.byteCode == "false":
        luacompileStr = luacompileStr + " --disable-compile"

    cmdStr = cmdStr + luacompileStr
    print( "cmd: " + cmdStr )
    subprocess.call( cmdStr, shell=True )

    # remove .lua
    for dirpath, dirnames, filenames in os.walk(tmpFolder):
            for filename in filenames:
                directFilePath = os.path.normpath(os.path.join(dirpath,filename))
                folderPath,filePath = os.path.split(directFilePath)
                if ".luac" in filePath:
                    newName = filePath.replace( ".luac", ".dat" )
                    os.rename( folderPath + '/' + filePath, folderPath + '/' + newName )
                    continue
                elif ".lua" in filePath:
                    os.remove( directFilePath )
